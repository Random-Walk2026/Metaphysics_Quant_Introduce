# MetaphysicsQuant - 工程化中國術數分析平台（繁體中文）
...
> **展示說明**：本倉庫只用於專案介紹與成果展示，不包含任何專案源碼、資料檔案、案例資料庫或 API 設定。

MetaphysicsQuant 不是一個只會顯示四柱、星盤或卦盤的排盤工具。它把中國傳統術數中最難工程化的部分做成了可運行的軟件系統：曆法換算、干支推導、神煞與格局規則、古籍條文檢索、專題化報告、AI 二次解讀、SQLite 案例庫、Markdown/TXT 匯出和桌面端完整工作流。

當前穩定主線是 `mainAppUI.py` 的 PySide6 桌面應用；`mainAppUI_modern.py` 是註冊表驅動的現代化外殼；`mainWebUI.py` 和 `web_platform/` 保留為 Web 方向程式碼。
...
這個專案的定位是“傳統術數知識庫 + 規則引擎 + 桌面分析系統”，重點是把大量分散在古籍、民間訣法和現代排盤經驗裡的規則，轉成可複查、可儲存、可擴充的程式輸出。
...
| 神煞與格局 | `SZBZ_1Gz.py` 中有 330 個方法，其中約 180 個與神煞、貴格、凶格、刑沖合會、串宮壓運等判定直接相關。 |
...
| 西文數理 | Pythagorean、Cheiro/Chaldean、Vedas 三體系合併報告，支援英文姓名與選年分析。 |
| 神數系統 | 鐵板神數、邵子神數、太素/太玄類數字推演，支援條文檢索、原局、大運、流年和綜合分析。 |
...
專案輸出不是一句“吉凶如何”的短評，而是可展開的結構化資料。例如當前倉庫中的四柱原局報告樣本已經達到 3 萬字量級，包含排盤資訊、七柱專題、古籍條文、神煞組合、運限和 AI 解讀入口。是否最終採用某條斷語，仍由使用者結合現實背景判斷。

## 介面截圖

以下截圖來自當前桌面端程式，按主要專題模組展示實際介面形態。
...
<table>
  ...
  <tr>
    <td align="center"><strong>鐵板神數 / 邵子神數</strong><br><img src="../image/esoteric_numerology.png" alt="鐵板神數與邵子神數介面" width="900"></td>
  </tr>
  <tr>
...
四柱八字是當前倉庫最重的核心模組：`four_pillars_destiny/` 約 2.88 萬行 Python，15 個 JSON 數據文件。它的價值不在“排出八個字”，而在排盤之後的大規模規則整理和報告生成。

### 1. 兩套入口

- `四柱八字`：輸入姓名、性別、公曆/農曆年月日時分，自動換算四柱、起運、大運、流年、流月。
- `四柱批命`：手動輸入年柱、月柱、日柱、時柱、大運、流年、小限、流月，適合古籍案例、已知八字反查和課堂推演。

### 2. 干支規則底座

`four_pillars_destiny/SZBZ_1Gz.py` 是四柱系統的規則地基，涵蓋：

- 天干、地支、藏干、五行、陰陽、六十甲子、纳音、旬空。
- 十神、十二長生、祿命長生、纳音長生、四貴四平四忌。
- 天乙貴人、福星貴人、太极貴人、文昌貴人、天德、月德、天喜、驛馬、華蓋、金輿、學堂、詞館、截路空亡、鬼門關、沐浴殺、暴敗、自縊、四廢、官符、死符、病符、血光、離鄉、劫煞、亡神、羊刃、元辰、空亡、咸池、天羅地網、孤辰寡宿等大量神煞與格局判定。
- 串宮壓運十二神：太歲、青龍、喪門、六合、官符、小耗、大耗、朱雀、白虎、貴神、吊客、病符，並實現神煞變化規則。
- 盲師神煞與民間經驗規則，例如四窮日、莊殺、羅殺、忌殺、八敗、懸梁殺、胞胎殺、鐵刑殺、天掃星、地掃星、追魂、鏟度等。

這裡適合展示專案實力：這些不是 UI 文案，而是被拆成函式和資料結構的規則引擎。
...
這些專題不是單一流派硬套，而是按來源分層拼接，例如蘇氏盲派、民間盲派、五行精紀、三命通會、玉照神應真經、梁派理論、鬼谷子分定經、三世演禽等。README 裡強調“實事求是”：專案提供可追溯的規則輸出，不宣稱替代現實決策。

### 4. 古籍與數據來源

四柱模組的 `data/` 目錄儲存了多類規則與條文數據：
...
程式碼中也直接引用《淵海子平》《蘭台妙選》《玉照神應真經》《永樂大典》《天元巫咸經》等來源或規則名。展示時可以說：這是一個把多種典籍、訣法和程式規則合併成專題報告的工程專案。
...
- 從當前年份起的 10 年流年中批：`zpln_num()`
- 流年與大運交接分段、流月與大運交接分段
- 可選紫微斗數聯動，生成原局、大限、流年排盤圖並儲存圖片

倉庫中已有脫敏範例報告，如 `character_case/CaseReports/範例案例/範例案例_四柱八字_原局分析_20260421_043836.md`，字元量約 3.6 萬；`character_case/CaseReports/A參考模板/範例案例_四柱八字_原局分析_20260421_031317.txt` 約 3.0 萬字符。這說明系統實際可以輸出長篇結構化報告，而不是停留在功能設想。

## 其他術數模組

### 紫微斗數

`purple_star_astrology/` 實現十二宮排盤、主星辅星雜曜、四化、星曜亮度、大限、流年、流月和排盤圖。`Star_Combination_Matcher.py` 負責從 JSON 數據中匹配單星、組合星和男女差異化解釋。專題數據涵蓋性格、財富、婚姻、事業、健康、父母、母親、兄弟、子女、田宅、遷移、交友等。

### 易經解卦與六爻

`hexagram_divination/` 支援時間起卦、三數起卦、玄機數、玄卦輸入、本卦、變卦、動爻、六親、世應、六神、納甲和流年解卦。`notion/` 下儲存 64 卦 Markdown 文件，`data/` 下儲存《易經》資料、小六壬、諸葛神數、干支神數、太極神數、牙牌神數等映射。

### 擇日合婚

`auspicious_date/` 不是簡單黃曆查詢，而是按主題篩選日期。已涵蓋婚嫁、納采問名、安床、上官赴任、臨政親民、開業開市、求財、立契交易、提車、宴會、求學、考試、入學、拜師、出行、求醫、入宅、祭祀、求子等場景。規則中可見《協紀辨方書》《玉匣記》和其他民俗擇日條文。

### 姓名學與西文數理

`nameology/` 涵蓋中文姓名學和西文數理：

- 中文姓名學：康熙/新華筆畫、五格、三才、八十一數理、五行、陰陽、生肖、專題斷語。
- 起名：個人起名、公司起名，可按喜用五行、規避五行、行業數據篩選。
- 測字與號碼：三字測字、手機號/車牌/編號數理分析。
- 西文數理：Pythagorean、Cheiro/Chaldean、Vedas 三體系合併輸出。
- 數據層：姓名字庫、康熙/新華筆畫、三才、五格、八十一數理、行業、西文數理資料，以及詩經、楚辭、論語、唐詩、宋詩、宋詞等起名語料。

### 鐵板神數、邵子神數、太素/太玄

`esoteric_numerology/` 提供鐵板神數、邵子神數和太素/太玄類數字推演。它與西文數理不同，更偏向傳統術數中的生辰、干支、刻分、卦象、聲音和條文檢索。邵子神數支援原局、大運、流年和综合報告；鐵板神數支援刻分、考刻和條文輸出。

## AI 分析與報告系統

專案没有把 AI 当成“神秘判斷器”，而是把 AI 放在工程链路的最後一層：先由規則系統生成可追溯的原始報告，再按專題拆分給模型做解釋、歸納和現實建議。

`ai/` 模組的能力包括：

- 供應商抽象：OpenAI、Claude、Google Gemini、OpenRouter、DeepSeek、DashScope 千問、SiliconFlow。
- OpenAI-compatible 支援：可接入 OpenRouter、DeepSeek、硅基流動等兼容 chat/completions 的服務。
- LangChain/LangGraph 風格工作流：有依賴時使用圖式呼叫，缺依賴時回退 REST。
- 專題拆分：`topic_schema.py` 根據報告模板拆分原局、大運、流年、流月、邵子等不同结構。
- 提示詞管理：`modern_prompts.py` 和 `prompt_manager.py` 將輸出約束到“現實建議、風險邊界、證據來源”，避免純術士化口吻。
- 金鑰管理：真實 API Key 走 `AI/.env` 或環境變數，不應提交到倉庫。

這部分的展示價值是：專案不是單純“玄學脚本”，而是傳統規則引擎與現代 LLM 工作流的結合。

## 案例管理與匯出

`character_case/` 是統一案例系統，默認資料庫為 `character_case/cases.db`。它让專案從“算一次”升級為“長期研究與歸檔”：

- 按系統儲存案例：四柱八字、四柱批命、紫微斗數、易經解卦、擇日合婚、姓名號碼、鐵板神數、邵子神數。
- 分類、搜尋、備註、排序、拖曳、編輯。
- 從案例管理頁重新載入到對應分析介面。
- JSON 備份和 Excel 匯入匯出。
- `unified_formatter.py` 將分析结果整理為 Markdown 或 TXT，針對八字、紫微、六爻、邵子等系統做格式識別與排版最佳化。

注意：`character_case/cases.db` 是本機使用者數據文件，不應随意涵蓋或刪除。
...
- 規則引擎：大量傳統規則被拆成可讀函式、數據表和 JSON 條文庫。
...
- Web 預研：`web_platform/` 保留 FastAPI/Vue/部署設定，說明專案具備從桌面端遷移到 Web 平台的基礎。

## 專案架構

MetaphysicsQuant 的架構重點是“桌面入口統一調度，核心術數模組獨立計算，曆法、案例、AI 和 UI 能力作為共享支撐層”。這让每個術數系統既能單獨執行，又能接入統一案例庫、統一匯出和 AI 專題解讀。
...
│ ├─ dialogs/: 不同術數系統的案例輸入/編輯彈窗                         │
│ └─ unified_formatter.py: Markdown/TXT 匯出                          │
│                                                                    │
│ ai/                                                                │
│ ├─ ai_analysis.py: AI 分析 UI、執行緒、快取與專題流程                  │
│ ├─ config.py / .env: 供應商設定與本地 API Key                       │
│ ├─ analysis_graph.py: LangGraph 風格呼叫流程                        │
│ ├─ modern_prompts.py / prompt_manager.py: Prompt 契約與相容層        │
...
1. `mainAppUI.py` 建立穩定的分頁桌面端；`mainAppUI_modern.py` 建立註冊表驅動的現代化外殼。
...
按當前本地倉庫統計，不含 `.git`、`__pycache__`、`.pytest_cache` 和 `.vscode`：
...
|---|---|---|---|
| 四柱八字 | `four_pillars_destiny/README.md` | 八字原局、大運、流年、流月、干支手輸和古籍規則數據。 | `四柱八字` / `四柱批命` |
| 紫微斗數 | `purple_star_astrology/README.md` | 紫微排盤、十二宮、星曜組合、大限、流年和排盤圖。 | `紫微斗數` |
...
| 通用 UI | `ui_common/README.md` | 共享執行緒、结果彈窗、現代化外殼、模組注册、導航和圖标主題。 | 底層支撑 |
| Web 平台 | `web_platform/README.md` | FastAPI/Vue Web 平台、後端介面、前端頁面和部署结構。 | Web 入口 |
...
AI 能力需要在 `AI/.env` 或系統環境變數中設定對應供應商金鑰。不要提交真實 API Key。

## 免責聲明

MetaphysicsQuant 用于傳統文化數位化、規則工程、軟體架構實踐和研究展示。專案輸出來自程式規則、數據文件和 AI 總結，不構成醫療、法律、投資、婚姻或其他現實決策建議。傳統術數內容應作為文化研究和個人參考，不應替代專業意見或事實調查。
